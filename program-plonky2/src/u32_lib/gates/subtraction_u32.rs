...
impl<F: RichField + Extendable<D>, const D: usize> Gate<F, D> for U32SubtractionGate<F, D> {
    ...
    fn eval_unfiltered(&self, vars: EvaluationVars<F, D>) -> Vec<<F as Extendable<D>>::Extension> {
        ...
        for i in 0..self.num_ops {
            ...
            let mut combined_limbs = <F as Extendable<D>>::Extension::ZERO;
            let limb_base =
            ...
            constraints
                .push(output_borrow * (<F as Extendable<D>>::Extension::ONE - output_borrow));
        }
    ...
    fn eval_unfiltered_circuit(
        ...
        for i in 0..self.num_ops {
            ...
            let base = builder.constant_extension(
                <F as Extendable<D>>::Extension::from_canonical_u64(1 << 32u64),
            );
            ...
            let limb_base = builder.constant_extension(
                <F as Extendable<D>>::Extension::from_canonical_u64(1u64 << Self::limb_bits()),
            );
            for j in (0..Self::num_limbs()).rev() {
                let this_limb = vars.local_wires[self.wire_ith_output_jth_limb(i, j)];
...
mod tests {
    ...
    fn test_gate_constraint() {
        ...
        let inputs_x = vec![0, u32::MAX as u64, 0x1234_5678];
        let inputs_y = vec![1, 0x8000_0000, 0x1234_5677];
        let borrows = vec![0, 1, 1];
