...
impl<F: RichField + Extendable<D>, const D: usize> Gate<F, D> for ComparisonGate<F, D> {
    ...
    fn eval_unfiltered(&self, vars: EvaluationVars<F, D>) -> Vec<<F as Extendable<D>>::Extension> {
        ...
        let first_chunks: Vec<<F as Extendable<D>>::Extension> = (0..self.num_chunks)
            .map(|i| vars.local_wires[self.wire_first_chunk_val(i)])
            .collect();
        let second_chunks: Vec<<F as Extendable<D>>::Extension> = (0..self.num_chunks)
            .map(|i| vars.local_wires[self.wire_second_chunk_val(i)])
            .collect();

        let first_chunks_combined = reduce_with_powers(
            &first_chunks,
            <F as Extendable<D>>::Extension::from_canonical_usize(1 << self.chunk_bits()),
        );
        ...
        let mut most_significant_diff_so_far = <F as Extendable<D>>::Extension::ZERO;

        for i in 0..self.num_chunks {
            // Range-check the chunks to be less than `chunk_size`.
            let first_product: <F as Extendable<D>>::Extension = (0..chunk_size)
                .map(|x| first_chunks[i] - <F as Extendable<D>>::Extension::from_canonical_usize(x))
                .product();
            let second_product: <F as Extendable<D>>::Extension = (0..chunk_size)
                .map(|x| {
                    second_chunks[i] - <F as Extendable<D>>::Extension::from_canonical_usize(x)
                })
                .product();
            ...
            constraints.push(
                difference * equality_dummy - (<F as Extendable<D>>::Extension::ONE - chunks_equal),
            );
            ...
            most_significant_diff_so_far = intermediate_value
                + (<F as Extendable<D>>::Extension::ONE - chunks_equal) * difference;
        }
        ...
        let most_significant_diff_bits: Vec<<F as Extendable<D>>::Extension> =
            (0..self.chunk_bits() + 1)
        ...
        for &bit in &most_significant_diff_bits {
            constraints.push(bit * (<F as Extendable<D>>::Extension::ONE - bit));
        }

        let bits_combined = reduce_with_powers(
            &most_significant_diff_bits,
            <F as Extendable<D>>::Extension::TWO,
        );
        let two_n = <F as Extendable<D>>::Extension::from_canonical_u64(1 << self.chunk_bits());
    ...
    fn eval_unfiltered_circuit(
        ...
        for i in 0..self.num_chunks {
            ...
            for x in 0..chunk_size {
                let x_f = builder
                    .constant_extension(<F as Extendable<D>>::Extension::from_canonical_usize(x));
        ...
        let two_n = builder.constant_extension(
            <F as Extendable<D>>::Extension::from_canonical_u64(1 << self.chunk_bits()),
        );
...
mod tests {
    ...
    fn test_gate_constraint() {
        ...
        let first_input_u64 = 0x12_3456_789a;
        let second_input_u64 = 0x34_5678_9abc;
