...
impl<F: RichField + Extendable<D>, const D: usize> Gate<F, D> for U32AddManyGate<F, D> {
    ...
    fn eval_unfiltered(&self, vars: EvaluationVars<F, D>) -> Vec<<F as Extendable<D>>::Extension> {
        ...
        for i in 0..self.num_ops {
            let addends: Vec<<F as Extendable<D>>::Extension> = (0..self.num_addends)
                .map(|j| vars.local_wires[self.wire_ith_op_jth_addend(i, j)])
                .collect();
            let carry = vars.local_wires[self.wire_ith_carry(i)];

            let computed_output = addends
                .iter()
                .fold(<F as Extendable<D>>::Extension::ZERO, |x, &y| x + y)
                + carry;
            ...
            let mut combined_result_limbs = <F as Extendable<D>>::Extension::ZERO;
            let mut combined_carry_limbs = <F as Extendable<D>>::Extension::ZERO;
            let base =
    ...
    fn eval_unfiltered_circuit(
        ...
        for i in 0..self.num_ops {
            ...
            let base: <F as Extendable<D>>::Extension = F::from_canonical_u64(1 << 32u64).into();
...
mod tests {
    ...
    fn test_gate_constraint() {
        ...
        let addends: Vec<Vec<_>> = (0..NUM_U32_ADD_MANY_OPS)
            .map(|op| {
                (0..NUM_ADDENDS)
                    .map(|i| 0x9e37_79b9u32.wrapping_mul((op * NUM_ADDENDS + i + 1) as u32) as u64)
                    .collect()
            })
            .collect();
        let carries = vec![0, 7, 15];
