...
impl<F: RichField + Extendable<D>, const D: usize> Gate<F, D> for U32ArithmeticGate<F, D> {
    ...
    fn eval_unfiltered(&self, vars: EvaluationVars<F, D>) -> Vec<<F as Extendable<D>>::Extension> {
        ...
        for i in 0..self.num_ops {
            ...
            let combined_output = {
                let base = <F as Extendable<D>>::Extension::from_canonical_u64(1 << 32u64);
                let one = <F as Extendable<D>>::Extension::ONE;
                let u32_max = <F as Extendable<D>>::Extension::from_canonical_u32(u32::MAX);
            ...
            let mut combined_low_limbs = <F as Extendable<D>>::Extension::ZERO;
            let mut combined_high_limbs = <F as Extendable<D>>::Extension::ZERO;
            let midpoint = Self::num_limbs() / 2;
            let base =
                <F as Extendable<D>>::Extension::from_canonical_u64(1u64 << Self::limb_bits());
            for j in (0..Self::num_limbs()).rev() {
                let this_limb = vars.local_wires[self.wire_ith_output_jth_limb(i, j)];
                let max_limb = 1 << Self::limb_bits();
                let product = (0..max_limb)
                    .map(|x| this_limb - <F as Extendable<D>>::Extension::from_canonical_usize(x))
                    .product();
    ...
    fn eval_unfiltered_circuit(
        ...
        for i in 0..self.num_ops {
            ...
            // Check canonicity of combined_output = output_high * 2^32 + output_low
            let combined_output = {
                let base: <F as Extendable<D>>::Extension =
                    F::from_canonical_u64(1 << 32u64).into();
                let base_target = builder.constant_extension(base);
                let one = builder.one_extension();
                let u32_max = builder.constant_extension(
                    <F as Extendable<D>>::Extension::from_canonical_u32(u32::MAX),
                );
            ...
            for j in (0..Self::num_limbs()).rev() {
                ...
                for x in 0..max_limb {
                    let x_target = builder.constant_extension(
                        <F as Extendable<D>>::Extension::from_canonical_usize(x),
                    );
...
mod tests {
    ...
    fn test_gate_constraint() {
        ...
        let multiplicands_0 = vec![0, 0x1234_5678, u32::MAX as u64];
        let multiplicands_1 = vec![u32::MAX as u64, 0x9abc_def0, 1];
        let addends = vec![17, u32::MAX as u64, 0x8000_0000];
