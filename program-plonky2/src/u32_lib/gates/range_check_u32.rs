...
impl<F: RichField + Extendable<D>, const D: usize> Gate<F, D> for U32RangeCheckGate<F, D> {
    ...
    fn eval_unfiltered(&self, vars: EvaluationVars<F, D>) -> Vec<<F as Extendable<D>>::Extension> {
        ...
        let base = <F as Extendable<D>>::Extension::from_canonical_usize(Self::BASE);
        for i in 0..self.num_input_limbs {
            ...
            for aux_limb in aux_limbs {
                constraints.push(
                    (0..Self::BASE)
                        .map(|i| {
                            aux_limb - <F as Extendable<D>>::Extension::from_canonical_usize(i)
                        })
                        .product(),
                );
            }
