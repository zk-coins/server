...
use crate::u32_lib::gates::comparison::ComparisonGate;
...
mod tests {
    ...
    fn test_list_le(size: usize, num_bits: usize) -> Result<()> {
        ...
        let lst1: Vec<u64> = (0..size)
            .map(|i| {
                (0x9e37_79b9_7f4a_7c15u64.wrapping_mul(i as u64 + 1)) & ((1u64 << num_bits) - 1)
            })
            .collect();
        let lst2: Vec<u64> = (0..size)
            .map(|i| {
                (0xd1b5_4a32_d192_ed03u64.wrapping_mul(i as u64 + 3)) & ((1u64 << num_bits) - 1)
            })
            .collect();
