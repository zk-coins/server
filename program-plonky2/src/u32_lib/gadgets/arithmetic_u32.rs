...
use crate::u32_lib::gates::add_many_u32::U32AddManyGate;
use crate::u32_lib::gates::arithmetic_u32::U32ArithmeticGate;
use crate::u32_lib::gates::subtraction_u32::U32SubtractionGate;
...
mod tests {
    ...
    use crate::u32_lib::witness::WitnessU32;

    #[test]
    fn u32_mul_uses_custom_gate_count() -> Result<()> {
        const D: usize = 2;
        ...
        let (low, high) = builder.mul_u32(a, b);
        let gate_count = builder.num_gates();

        println!("one u32 multiply gate count: {gate_count}");
        assert!(
            (1..=3).contains(&gate_count),
            "expected one u32 multiply to use 1-3 gates, got {gate_count}"
        );

        builder.register_public_input(low.0);
        builder.register_public_input(high.0);

        let a_value = 0xffff_fffeu32;
        let b_value = 0x1234_5678u32;
        let expected = (a_value as u64) * (b_value as u64);
        let mut pw = PartialWitness::new();
        pw.set_u32_target(a, a_value)?;
        pw.set_u32_target(b, b_value)?;

        let data = builder.build::<C>();
        let proof = data.prove(pw)?;
        assert_eq!(
            proof.public_inputs[0],
            F::from_canonical_u32(expected as u32)
        );
        assert_eq!(
            proof.public_inputs[1],
            F::from_canonical_u32((expected >> 32) as u32)
        );
        data.verify(proof)
    }
    ...
    pub fn test_add_many_u32s() -> Result<()> {
        ...
        let mut sum = 0u64;
        for i in 0..NUM_ADDENDS {
            let x = (0x9e37_79b9u32).wrapping_mul(i as u32 + 1);
            sum += x as u64;
